...
void GameClient::gameOver() {
	...
	colorLayer->setPosition(Vec2(visibleSize.width/2-240, 0));
	this->addChild(colorLayer,15);
	...
	colorLayer->addChild(lv,1);

	auto size = Size(300, 100);


	Label* label = Label::createWithBMFont("fonts/futura-48.fnt", "Rankings");
	label->setColor(cocos2d::Color3B(255, 255, 255));
	label->setPosition(Vec2(visibleSize.width / 2, visibleSize.height -label->getHeight()));
	...
	lv->addChild(layout,2);
	cout<< rankings[0].name + "  " + std::to_string(rankings[0].score);
	//for (int i = 0; i < 3; ++i)
	//{
	...
	//	Label* label1 = Label::createWithBMFont("fonts/futura-48.fnt", rankings[i].name+"  "+std::to_string(rankings
	//	[i].score));
	//	
	//	label1->setColor(cocos2d::Color3B(255, 255, 255));
	//	label1->setPosition(Vec2(visibleSize.width / 2, visibleSize.height - label1->getHeight()-label->getHeight()));

	//	auto layout1 = cocos2d::ui::Layout::create();
	//	layout1->setBackGroundImageScale9Enabled(true);
	//	layout1->setContentSize(size);
